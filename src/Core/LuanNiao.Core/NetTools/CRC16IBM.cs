...
{
    /// <summary>
    /// the CRC16/IMB's .NET version
    /// </summary>
    public static class CRC16IBM
    {

        private static readonly UInt16[] _table ={ 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241, 0xC601, 0x06C0, 0x0780, 0xC741,
...
0x4100, 0x81C1, 0x8081, 0x4040};
        ...
        public static byte[] GetCRC16(Span<byte> data)
        {
            ...
            {
                crc = (UInt16)(((UInt16)(crc >> 8)) ^ _table[(crc ^ item) & 0xff]);
            }
        ...
        {
            UInt16 crc = 0x0000;
            foreach (byte item in source.Slice(beginIndex, endIndex))
            {
                crc = (UInt16)(((UInt16)(crc >> 8)) ^ _table[(crc ^ item) & 0xff]);
            }
            BitConverter.GetBytes(crc).AsSpan().CopyTo(source.Slice(crcIndex));
        }
    }
}
